...
std::vector<std::vector<Landmark>> hand_points; // holds list of hands, each with 21 landmark points (loaded from JSON)
...
// Called by OpenGL every time the screen needs to be redrawn
void display()
{
    ...
    glLoadIdentity(); // reset last matrix
    ...
    for (const auto &hand : hand_points)
    {
    ...
    for (const auto &hand : hand_points)
    {
        ...
        {
            const auto &a = hand[connections[i][0]];
            const auto &b = hand[connections[i][1]];
...
int main(int argc, char **argv)
{
    ...
    // starts the main render loop. From here on, GLUT takes over.
